...
import { ShareScreenListener } from './share-screen';
...
export class SocketIoSubscriber {
  ...
  public subscribe(io: Server): void {
    io.on('connection', (socket) => {
      ...
      const createListener = new CreateRoomListener(
        socket,
      ...
      const deleteListener = new DeleteRoomListener(
        socket,
        this.roomsRepository
      );
      const disconnectListener = new DisconnectListener(io, socket);
      const shareScreenListener = new ShareScreenListener(
        socket,
        this.roomsRepository
      );
    });
